...
```{r}
# for (folio in rawData_2009){ ...complete process } # after each loop a randow pause
Rowcount <- 1
folio_test <- rawData_2009[1]
as.character(folio_test[1])
```

# 2. Fill in the form to make the query with FOLIO keys
```{r}
folio <- as.character(folio_test)
searchID<-'//*[(@id = "Usuario")]' # form to fill in with the folio number
...
ConsButton <- '//td//td//img' # xpath to the "consultar" button
...
```{r}
tableID <- '/html/body/table[3]' # xpath to the general information table
...
DataBase$Grado[[Rowcount]] <- (levels(col1Values)[7]);DataBase
DataBase$Grupo[[Rowcount]] <- (levels(col1Values)[2])
DataBase$Turno[[Rowcount]] <- (levels(col1Values)[6])
DataBase$TipoDeEscuela[[Rowcount]] <- (levels(col1Values)[4])
DataBase$NombreDeLaEscuela[[Rowcount]] <- (levels(col1Values)[5])
DataBase$CCT[[Rowcount]] <- as.character(levels(col2Values)[1])
DataBase$Entidad[[Rowcount]] <- as.character(levels(col2Values)[3])
DataBase$GradoDeMarginacion[[Rowcount]]<- as.character(levels(col2Values)[4])
```
...
subject <- "R scraping for ENLACE 2009"
body <- "R scraping for ENLACE 2009 is has stopped for a reason."
