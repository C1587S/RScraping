...
folio_test <- as.array(t(rawData_2012[1:3]))
remDr$navigate(url_raw)
for (folioID in seq(1,3,by=1)){ #rawData_2012
...
# 7 Respuesta de mi hija(o) en Matemáticas
  ...
  webFramesMat <- remDr$findElements(using = 'xpath', value = FrameEspID)
...
### After the loop finishes, write a CSV file
 write.csv(DataBase, file="CreatedData/2012/DataBase_ENLACE2012_Test.csv", row.names = FALSE)
