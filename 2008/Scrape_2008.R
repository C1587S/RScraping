...
folio_test <- as.array(t(rawData_2008[1:3]))
remDr$navigate(url_raw) 
for (folioID in seq(1,3,by=1)){ #rawData_2008
  remDr$navigate(url_raw)
...
###############################
   for (i in seq(1,length(pregs_esp), by=1)){
     ...
     Sys.sleep(0.8)
     preg_correctaID <- remDr$findElement(value = "/html/body/span/table[3]/tbody/tr[1]/td/b")
     ...
     DataBase[folioID, nroPregunta_int+11] <-as.character(correctaInfo) # Esp correcta (+271 is the correction to math the position in the DF)
     DataBase[folioID, nroPregunta_int+141] <-as.character(marcadaInfo) # Esp marcada (+401 is the correction to math the position in the DF)
  ...
  for (i in seq(1,length(pregs_mat), by=1)){
    ...
    elemento_preg <- remDr$findElement(value = preguntaID)
    remDr$executeScript("arguments[0].click();", list(elemento_preg))
    Sys.sleep(0.8) # This part is mandatory, we need to include a pause to load the data correctly
    preg_correctaID <- remDr$findElement(value = "/html/body/span/table[3]/tbody/tr[1]/td/b")
    correctaInfo    <- preg_correctaID$getElementText()
    # when there is no answer 
    preg_marcadaID <- tryCatch({remDr$findElement(value = "/html/body/span/table[3]/tbody/tr[2]/td/b")}, silent=TRUE,error=function(err) NA)
...
### After the loop finishes, write a CSV file
 write.csv(DataBase, file="/Users/c1587s/Dropbox/Webscrape_Puntajes/CreatedData/2008/DataBase_ENLACE2008_Test.csv", row.names = FALSE)
